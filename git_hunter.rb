...
if ARGV[0] == 'run'
  if ARGV[1].nil?
    File.open('user_list.txt').read.each_line do |line|
      args = line.split(' ')
      GitHunterCore.new(args[1].split('/').last, args[0]).run
    end
    GitHunterRenderer.new.run
  elsif ARGV[1] == 'global'
    GitHunterCore.new.run_global
    ...
    GitHunterCore.new(ARGV[1], ARGV[2]).run
    GitHunterRenderer.new(ARGV[1]).run
  elsif ARGV.size == 4
    GitHunterCore.new(ARGV[1], ARGV[2], ARGV[3]).run
    GitHunterRenderer.new(ARGV[1], ARGV[3]).run
  else
    puts HELP
    exit
  end
  ...
  if ARGV[1].nil?
    ...
    GitHunterRenderer.new(ARGV[1], ARGV[2]).run
  else
    puts HELP
    exit
  end
  ...
  if ARGV[1] == 'all'
    ...
    GitHunterCore.new.mark(ARGV[1])
  elsif ARGV.size == 3
    GitHunterCore.new.mark(ARGV[1], ARGV[2])
  else
    puts HELP
    exit
  end
